...

{
  ...
  */
  ...
  // step02();
  function step02() {
    const lines: string[] = ['1.234567890123456789'];
    console.log(lines[0]);
  }
  ...
  function boss(): void {
    ...
    const lines: string[] = ['3 4 1 2', '..#.', '#.##', '....'];
    const firstLineValues: string[] = lines[0].split(" ");
    const h: number = parseInt(firstLineValues[0]);
    const w: number = parseInt(firstLineValues[1]);
    const r: number = parseInt(firstLineValues[2]);
    const c: number = parseInt(firstLineValues[3]);
    const mazeArray = [];
  ...
  function boss_gpt(): void {
    ...
    const maze: string[][] = lines.slice(1).map(line => line.split(""));
